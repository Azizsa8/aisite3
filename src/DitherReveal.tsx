...
/*
  Originkit "Dither Reveal", ported from components/originkit/ui/dither-reveal.tsx.
  Changes from the extracted source:
  - no Next.js "use client" directive, no stock default image (image is required);
  - pointer listeners attach to the closest <section> so the reveal follows the
    cursor even when it travels over the hero's own text layer.
*/
...
export default function DitherReveal(props: DitherRevealProps) {
  ...
  useEffect(() => {
    ...
    if (!canvas || !container || !imgUrl) return;
    ...
    // Track the pointer across the whole hero section, not just this layer —
    // the text sits above the canvas and would otherwise swallow the events.
    const evTarget: HTMLElement = container.closest("section") || container;
    ...
    evTarget.addEventListener("pointerenter", onEnter);
    evTarget.addEventListener("pointerleave", onLeave);

    function resize() {
      // cap DPR hard: the effect is a fullscreen fragment shader running
      // alongside the 3D scene, and its coarse dither look loses nothing at
      // lower density. 1x on phones, 1.5x elsewhere.
    ...
    return () => {
      ...
      evTarget.removeEventListener("pointerenter", onEnter);
      evTarget.removeEventListener("pointerleave", onLeave);
...
void main() {
    ...
    // weight luminance by alpha so transparent-background artwork (e.g. a
    // glowing mark on nothing) dithers on its glow gradient instead of
    // reading every anti-aliased pixel as full white
    float gray = dot(color.rgb, vec3(0.299, 0.587, 0.114)) * color.a;
    ...
    // dither field carries its own alpha so whatever sits behind the canvas
    // (the 3D scene) stays visible between the dots
    gl_FragColor = vec4(mix(ditherColor, color.rgb, revealAmount), mix(tone, color.a, revealAmount));
}
`;
