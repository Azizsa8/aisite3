...
function CoreScene({ progress, active, total }) {
  ...
  useFrame((state) => {
    ...
    if (core.current) {
      const cs = 0.55 + Math.sin(t * 0.9) * 0.03 + p * 0.22;
  ...
  return (
    <group ref={group}>
      ...
      {/* inner core — the data. Faceted and dim-emissive so it reads as a lit
          3D object, never a flat white silhouette blocking the page content. */}
      <mesh ref={core}>
        <icosahedronGeometry args={[1, 1]} />
        <meshStandardMaterial color="#1a1c20" emissive="#9aa1a8" emissiveIntensity={0.4}
          roughness={0.35} metalness={0.7} flatShading />
      </mesh>
